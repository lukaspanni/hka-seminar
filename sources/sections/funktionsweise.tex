...
%TODO: bild
Geräte ohne Internetverbindung (Accessories), wie zum Beispiel AirTags oder Geräte von Drittanbietern, können die Master Beacon Keys nicht über die iCloud Synchronisation erhalten.
Deshalb müssen diese Geräte auf andere Weise an einen initialen Schlüssel kommen.
Beim ersten Pairing mit einem Owner Device, werden vom Accessory und dem Owner Device gemeinsam ein \ac{ECC}-Schlüsselpaar und zwei symmetrische Schlüssel, in der Spezifikation als SKN und SKS bezeichnet \cite{Apple_FindMySpec}, berechnet.
Diese Schlüssel können ebenfalls verschlüsselt über die iCloud Keychain mit anderen Geräten des selben Besitzers synchronisiert werden.
So wird ermöglicht, dass der Besitzer empfangene Standortdaten auf allen Geräten entschlüsseln kann.
Abhängig vom Zustand in welchem sich das Accessory befindet, unterscheidet sich die Berechnung des Advertising Keys.
...
Durch die Verschlüsselung auf den Finder Devices müssen die Lost Devices alle 15 Minuten beziehungsweise nur alle 24 Stunden bei Accessories einen neuen Schlüssel ableiten, was durch die geringere Zahl der Operationen eine deutlich geringere Belastung für die Batterie darstellt.
